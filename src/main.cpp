...
struct Flags
{
	string myPath;					// path to the Krom exeutable that is now running
...
int main(int argc, char ** argv)
{
	...
	if (flags.flagError)
	{
		cout << "try 'Krom -h' for help" << endl;
		return 0;
	}
	
	if (flags.help)
	{
		cout << "Krom v" << VERSION_X << "." << VERSION_Y << "." << VERSION_Z << endl;
		cout << "usage: Krom [options] [source file] [options]" << endl;
		cout << "options: " << endl;
		cout << "-v, -version      display the version of Krom" << endl;
		...
		cout << "-cpp [file]       transpile to C++ and save the output in the given file (option makes segfault on some systems)" << endl;
		cout << "-bin [file]       transpile, compile with GCC and save the binary (makes segfault on some systems)" << endl;
	...
	if (flags.version)
	{
		cout << "Krom version " << VERSION_X << "." << VERSION_Y << "." << VERSION_Z << endl;
		return 0;
	}
	...
	{
		cout << "multiple source files specified, Krom does not currently support this" << endl;
		cout << "try 'Krom -h' for help" << endl;
		return 0;
	}
		...
		else
		{
			...
			if (!flags.binOutFile.empty() || flags.runCompiled)
			{
				...
				if (flags.runCompiled)
				{
					...
					cmd = "./"+binFileName + " --running-from-Krom " + str::join(cmdLineArgs, " ", false);
