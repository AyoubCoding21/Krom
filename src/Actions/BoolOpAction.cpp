...
public:
		...
		:ActionData(Bool, Void, Void)
	{
		...
		{
			throw KromError("AndAction created with first action that does not return Bool", INTERNAL_ERROR);
		}
		
		if (secondAction->getReturnType()!=Bool)
		{
			throw KromError("AndAction created with second action that does not return Bool", INTERNAL_ERROR);
		}
	}
...
public:
		...
		:ActionData(Bool, Void, Void)
	{
		...
		if (firstAction->getReturnType()!=Bool)
		{
			throw KromError("OrAction created with first action that does not return Bool", INTERNAL_ERROR);
		}
		
		if (secondAction->getReturnType()!=Bool)
		{
			throw KromError("OrAction created with second action that does not return Bool", INTERNAL_ERROR);
		}
	}
