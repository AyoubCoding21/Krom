...
public:
	...
	{
		throw KromError("tried to get the literal value of 'void;", INTERNAL_ERROR);
	}
...
public:
	...
	string getCompactString()
	{
		switch (primType)
		{
			...
			default:
				throw KromError("tried to make " + getString() + " compact", INTERNAL_ERROR);
		}
	}
	...
	string getCppLiteral(void * data, CppProgram * prog)
	{
		...
		switch (primType)
		{
			...
			default:
				throw KromError("tried to convert " + getString() + " to C++ code", INTERNAL_ERROR);
		}
	...
	size_t getSize()
	{
		switch (primType)
		{
			...
			default:
				throw KromError("tried to get size of " + getString(), INTERNAL_ERROR);
		}
	}
...
public:
	...
	Type actuallyIs(Type target)
	{
		if (target->getType()!=TUPLE)
		{
			...
			{
				throw KromError("actuallyIs called on tuple with target that is not tuple", INTERNAL_ERROR);
			}
		}
...
public:
	...
	string getCppLiteral(void * data, CppProgram * prog)
	{
		throw KromError("getCppLiteral called on whatev type, wich should not have happened", INTERNAL_ERROR);
	}
	
	size_t getSize()
	{
		throw KromError("getSize called on whatev type, wich should not have happened", INTERNAL_ERROR);
	}
	...
	Type getSubType()
	{
		throw KromError("getSubType called on whatev type, wich should not have happened", INTERNAL_ERROR);
	}
...
{
	throw KromError("getAllSubTypes called on type that was not a tuple", INTERNAL_ERROR);
}
...
Type TypeBase::actuallyIs(Type target)
{
	if (!matches(target))
	{
		throw KromError("actuallyIs called with type that doesn't match", INTERNAL_ERROR);
	}
	
	if (isWhatev())
	{
		throw KromError("actuallyIs not implemented properly for Whatev type", INTERNAL_ERROR);
	}
