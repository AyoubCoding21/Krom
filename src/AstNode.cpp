...
#include "../lib/msclStringFuncs.h"
#include "../lib/utils/stringDrawing.h"
...
void AstList::resolveAction()
{
	...
	{
		throw KromError("AstList given non void input", INTERNAL_ERROR, getToken());
	}
...
void AstExpression::resolveAction()
{
	if (!inLeftType->isVoid() || !inRightType->isVoid())
	{
		throw KromError("AstExpression given non void input", INTERNAL_ERROR, getToken());
	}
	
	if (rightIn->isType())
	{
		throw KromError("types must be declared as constants", SOURCE_ERROR, rightIn->getToken());
	}
	else if (center->isType() || center->isFunctionWithOutput() || leftIn->isType())
	{
		throw KromError("a function implementation got into an expression node somehow", INTERNAL_ERROR, center->getToken());
	}
	else
	{
...
void AstConstExpression::resolveConstant()
{
	if (!inLeftType->isVoid() || !inRightType->isVoid())
	{
		throw KromError("AstConstExpression given non void input", INTERNAL_ERROR, getToken());
	}
...
void AstOpWithInput::resolveAction()
{
	if (token->getOp()==ops->ifOp)
	{
		...
		if (leftIn.empty())
		{
			throw KromError("'?' must have a conditional to its left", SOURCE_ERROR, token);
		}
		else if (leftIn.size()!=1)
		{
			throw KromError("'?' can only have one conditional to its left", SOURCE_ERROR, token);
		}
		...
		if (rightIn.empty())
		{
			throw KromError("'?' must have a statement to its right", SOURCE_ERROR, token);
		}
		...
		{
			throw KromError("'?' can only have 1 or 2 '|' seporated expressions to its right", SOURCE_ERROR, token);
		}
	}
	else if (token->getOp()==ops->loop)
	{
		...
		if (rightIn.size()>1)
		{
			throw KromError("'@' followed by multiple expressions", SOURCE_ERROR, token);
		}
		
		if (leftIn.size()==0)
		{
			throw KromError("condition needed before '@'", SOURCE_ERROR, token);
		}
		else if (leftIn.size()==1)
		{
		...
		{
			throw KromError("chain of length "+to_string(leftIn.size())+"preceding '@', it should be length 1-3", SOURCE_ERROR, token);
		}
	...
	else if (token->getOp()==ops->andOp || token->getOp()==ops->orOp)
	{
		...
		{
			throw KromError("'"+token->getOp()->getText()+"' can not be sent '|' separated sequence", SOURCE_ERROR, getToken());
		}
		
		if (leftIn.size()!=1 || rightIn.size()!=1)
		{
			throw KromError("'"+token->getOp()->getText()+"' must be given a left and right input", SOURCE_ERROR, getToken());
		}
		...
		{
			throw KromError("'"+token->getOp()->getText()+"' can only be used with Bools", SOURCE_ERROR, leftIn[0]->getToken());
		}
		
		if (rightAction->getReturnType()!=Bool)
		{
			throw KromError("'"+token->getOp()->getText()+"' can only be used with Bools", SOURCE_ERROR, rightIn[0]->getToken());
		}
	...
	{
		throw KromError("AstOpWithInput::resolveAction called for token '"+token->getOp()->getText()+"', which it shouldn't have been", INTERNAL_ERROR, token);
	}
	else
	{
		throw KromError("AstOpWithInput made with bad token '"+token->getText()+"'", INTERNAL_ERROR, token);
	}
}
...
void AstToken::resolveAction()
{
	...
	if (token->getType()==TokenData::IDENTIFIER || token->getType()==TokenData::OPERATOR)
	{
		...
		{
			throw KromError("non overloadable operator in AstToken, it should have been removed and processed by the parser", INTERNAL_ERROR, token);
		}
	...
	else if (token->getType()==TokenData::LITERAL || token->getType()==TokenData::STRING_LITERAL)
	{
		if (!inLeftType->isVoid() || !inRightType->isVoid())
		{
			throw KromError("a literal can not be given an input", SOURCE_ERROR, token);
		}
	...
	{
		throw KromError("AstToken givin invalid token '"+token->getText()+"' of type "+TokenData::typeToString(token->getType()), INTERNAL_ERROR, token);
	}
}
