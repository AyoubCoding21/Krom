...
Type NamespaceData::getType(string name, bool throwSourceError, Token tokenForError)
{
	...
	if (results.empty())
	{
		if (parent)
			...
			throw KromError("'"+name+"' type not found", SOURCE_ERROR);
		else
			return nullptr;
	}
	else if (results.size()!=1)
	{
		throw KromError("namespace has multiple defenitions of the same type '"+name+"'", INTERNAL_ERROR);
	}
	else if (results[0]->getReturnType()->getType()!=TypeBase::METATYPE)
	{
		throw KromError("node returning non meta type stored in namespace type map for type '"+name+"'", INTERNAL_ERROR);
	}
	else
	{
...
Action NamespaceData::getActionForTokenWithInput(Token token, Type left, Type right, bool dynamic, bool throwSourceError, Token tokenForError)
{
	...
	if (!matches.empty())
	{
		...
		{
			throw KromError("multiple matching instances of '"+token->getText()+"' found", SOURCE_ERROR, tokenForError);
		}
		else
		{
			return nullptr;
	...
	if (!matches.empty())
	{
		...
		{
			throw KromError("multiple whatev instances of '"+token->getText()+"' found", SOURCE_ERROR, tokenForError);
		}
		else
		{
			return nullptr;
	...
	{
		if (foundNodes)
			throw KromError("correct overload of '"+token->getText()+"' not found for types "+left->getString()+" and "+right->getString(), SOURCE_ERROR, tokenForError);
		else
			throw KromError("'"+token->getText()+"' not found", SOURCE_ERROR, tokenForError);
	}
	else
	{
		return nullptr;
